            ...
            <article class="border border-gray-200 rounded-lg hover:shadow-lg">
                @if ($post->getFeatured())
                    <a href="{{ $post->getpermalink() }}">
                        ...
                        {!! $post->getFeaturedImage('medium', $post->title, 'm-0 object-cover aspect-[3/2] rounded-t-md', '(max-width: 430px) 95vw, (max-width: 850px) 48vw, 280px', $loop->iteration < 5 ? 'eager' : 'lazy') !!}
                    </a>
                @endif
