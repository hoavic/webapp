...
class HomeController extends Controller
{
    public function default()
    {
        ...
        return view('coreblog::guest.home', [
            'newestPosts' => Post::with(['postMetas'])->where('status', 'published')->where('type', 'post')->latest()->limit(10)->get(),
            'categories' => Taxonomy::with(['term', 'ancestors'])->where('taxonomy', 'category')->get(),
            'contentStyle' => 'no-sidebar',
            'meta_tags' => $meta_tags,
            'products' => Product::with(['postMetas', 'variants'])->where('status', 'published')->where('type', 'product')->latest()->limit(3)->get(),
        ]);
    }
