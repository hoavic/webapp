        ...
        <table class="w-full text-right bg-white border border-gray-300">
            <thead>
                <tr>
                    ...
                    <th>File Name</th>
                    <th class="hidden md:table-cell">Mime_Type</th>
                    <th>Size</th>
                    <th>Action</th>
                </tr>
            </thead>
                    ...
                    <tr v-for="(item, index) in data" :key="index">
                        ...
                        <td class="text-left">
                            <a :href="route('admin.medias.edit', item.id)"
                                ...
                                <img :src="getUrl(item, 'thumbnail')" loading="lazy"/>
                            </a>
                        </td>
                        <td class="text-gray-600 text-sm text-left">{{ item.file_name }}</td>
                        <td class="hidden md:table-cell text-gray-600 text-sm">{{ stripHtml(item.mime_type) }}</td>
                        <td class="text-sm text-gray-400">{{ prettyBytes(item.size) }}</td>
                        <td>
                            <div class="flex gap-2 justify-end">

                                <a :href="getPermalink(item)" :title="'View ' + item.name"
                                ...
                                <button type="submit" @click.prevent="destroy(item.id)" :aria-label="'Delete ' + item.name" :title="'Delete ' + item.name"
                                    class="text-red-600">
