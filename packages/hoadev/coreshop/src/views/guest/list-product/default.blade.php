            ...
            <div class="md:p-4 lg:p-3 flex flex-col items-center border border-gray-100 md:rounded-lg hover:shadow-lg">
                <a href="{{ $post->getpermalink() }}" class="block w-full no-underline">
                    @if ($post->getFeatured())
                        ...
                        {!! $post->getFeaturedImage('large', $post->title, 'my-0 md:rounded object-cover aspect-square', '(max-width: 430px) 180px, 280px', $loop->iteration < $eager_load_from ? 'eager' : 'lazy') !!}
                    @else
